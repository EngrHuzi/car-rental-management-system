...
interface SatisfactionChartProps {
  data: Array<{ date: string; satisfaction: number; count: number }>;
}
...
export function SatisfactionChart({ data }: SatisfactionChartProps) {
        ...
        <Card className="relative border-0 shadow-xl overflow-hidden">
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-xl font-extrabold bg-gradient-to-r from-yellow-600 to-orange-600 bg-clip-text text-transparent">
                  Customer Satisfaction
                </CardTitle>
                <CardDescription className="font-medium">Average ratings over time</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-center h-[300px]">
              <div className="text-center">
                ...
                <p className="text-gray-600 font-semibold">No satisfaction data available</p>
              </div>
            </div>
          </CardContent>
        </Card>
  ...
  const totalFeedback = data.reduce((sum, d) => sum + d.count, 0);
  const maxRating = Math.max(...data.map((d) => d.satisfaction));
  ...
  const getStatusInfo = (avg: number) => {
    if (avg >= 4.5) return { label: 'Excellent', icon: ThumbsUp, gradient: 'from-green-600 to-emerald-600', bgGradient: 'from-green-50 to-emerald-50', border: 'border-green-200' };
    if (avg >= 3.5) return { label: 'Good', icon: Star, gradient: 'from-yellow-600 to-amber-600', bgGradient: 'from-yellow-50 to-amber-50', border: 'border-yellow-200' };
    return { label: 'Needs Work', icon: TrendingUp, gradient: 'from-red-600 to-orange-600', bgGradient: 'from-red-50 to-orange-50', border: 'border-red-200' };
  };

  const statusInfo = getStatusInfo(avgSatisfaction);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription className="font-medium text-gray-600">
                Average ratings over the last {data.length} days | Avg: <span className="font-bold text-orange-600">{avgSatisfaction.toFixed(2)}/5.0</span>
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            ...
            <div className="h-[300px] flex items-end gap-2 p-4 bg-gradient-to-br from-gray-50 to-white rounded-xl border border-gray-100">
              {data.map((item, index) => {
                const heightPercent = (item.satisfaction / 5) * 100;
                return (
                  <motion.div
                    key={index}
                    initial={{ height: 0 }}
                    ...
                    className={`flex-1 bg-gradient-to-t ${getRatingGradient(item.satisfaction)} rounded-t-lg transition-all relative group/bar shadow-lg hover:opacity-90`}
                    title={`${item.date}: ${item.satisfaction.toFixed(2)}/5 (${item.count} reviews)`}
                  >
                    {/* Tooltip */}
                    <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover/bar:block bg-gradient-to-r from-gray-900 to-gray-800 text-white text-xs rounded-lg py-2 px-3 whitespace-nowrap z-10 shadow-xl">
                      ...
                      <div className="text-yellow-300">{item.satisfaction.toFixed(2)}/5.0</div>
                      <div className="text-gray-300">{item.count} reviews</div>
                    </div>
                  </motion.div>
                );
              })}
            ...
            <div className="grid grid-cols-3 gap-4">
              <motion.div
                ...
                className="p-4 rounded-xl bg-gradient-to-br from-yellow-50 to-amber-50 border-2 border-yellow-200"
              >
                <div className="flex items-center gap-2 mb-2">
                  <Star className="w-4 h-4 text-yellow-600" />
                  <div className="text-xs font-bold text-gray-600">Average Rating</div>
                </div>
              ...
              <motion.div
                whileHover={{ y: -2, scale: 1.02 }}
                className="p-4 rounded-xl bg-gradient-to-br from-orange-50 to-red-50 border-2 border-orange-200"
              >
                <div className="flex items-center gap-2 mb-2">
                  <MessageSquare className="w-4 h-4 text-orange-600" />
                  <div className="text-xs font-bold text-gray-600">Total Reviews</div>
                </div>
            ...
            <div className="flex flex-wrap gap-3 p-4 bg-gradient-to-r from-gray-50 to-white rounded-xl border border-gray-200">
              <motion.div whileHover={{ scale: 1.05 }} className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-700">&lt;3.5 Poor</span>
              </motion.div>
              <motion.div whileHover={{ scale: 1.05 }} className="flex items-center gap-2">
                <div className="w-4 h-4 bg-gradient-to-r from-yellow-500 to-amber-500 rounded shadow-md"></div>
                <span className="text-xs font-bold text-gray-700">3.5-4.4 Good</span>
              </motion.div>
              <motion.div whileHover={{ scale: 1.05 }} className="flex items-center gap-2">
                <div className="w-4 h-4 bg-gradient-to-r from-green-500 to-emerald-500 rounded shadow-md"></div>
                <span className="text-xs font-bold text-gray-700">≥4.5 Excellent</span>
              </motion.div>
            </div>
          </div>
        </CardContent>
